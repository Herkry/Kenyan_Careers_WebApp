<?php

class Header
{
    function headerdisplay()
    {
        ?>
        
        <html lang ="en">
        <head>
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
        <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/css/bootstrap.min.css" integrity="sha384-Vkoo8x4CGsO3+Hhxv8T/Q5PaXtkKtu6ug5TOeNV6gBiFeWPGFN9MuhOf23Q9Ifjh" crossorigin="anonymous">
        <link rel="stylesheet" href="/Kenyan_Careers_WebApp/kenyan_careers_webapp/Assets/CSS/header.css">
        </head>

        <body>
            <!--Using flex for the header design 
            Check file: header.css 
            -->   
        <header class="site-header">
            
        <div class="row-md-3">
        <a href="#"><img height="90" width="260" src="/Kenyan_Careers_WebApp/kenyan_careers_webapp/Assets/Images/kenyacareer_vectorcopy.png" alt="kenyan_career_logo"></a>
        </div>
        
        <div class="btn-group">
            <h4 id="headerf"><b> Company SignUp</b></h4>
            <button class="btn btn-secondary dropdown-toggle" type="button" id="triggerId" data-toggle="dropdown" aria-haspopup="true"
                    aria-expanded="true">
                        More
            </button>
            <div class="dropdown-menu dropdown-menu-right" aria-labelledby="triggerId">
                <a class="dropdown-item" href="#">Login</a>
                <a class="dropdown-item" href="#">Apply for a job</a>
                <h5 class="dropdown-header">Our Services</h5>
                <a class="dropdown-item" href="#">Career News</a>
                <a class="dropdown-item" href="#">Explore Opportunities</a>
                <div class="dropdown-divider"></div>
                <a class="dropdown-item" href="#">Talk to Us</a>
            </div>
        </div>

    </header>

    <script src="https://code.jquery.com/jquery-3.4.1.slim.min.js" integrity="sha384-J6qa4849blE2+poT4WnyKhv5vZF5SrPo0iEjwBvKU7imGFAV0wwj1yYfoRSJoZ+n" crossorigin="anonymous"></script>
    <script src="https://cdn.jsdelivr.net/npm/popper.js@1.16.0/dist/umd/popper.min.js" integrity="sha384-Q6E9RHvbIyZFJoft+2mJbHaEWldlvI9IOYy5n3zV9zzTtmI3UksdQRVvoxMfooAo" crossorigin="anonymous"></script>
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/js/bootstrap.min.js" integrity="sha384-wfSDF2E50Y2D1uUdj0O3uMBJnjuUD4Ih7YwaYd1iqfktj0Uod8GCExl3Og8ifwB6" crossorigin="anonymous"></script>    
    
    </body>
    </html>
    <?php
    }

}
    

?>
